    ...
    protected boolean insert(RTNode node)
    {
        if(usedSpace < rtree.getNodeCapacity())
        {
            ...
            for(int m=0;m<n.children.size();m++)
            {
                ...
                sn += n.datas[m] + n.children.get(m).nodeHash + n.bF[0].getBits().toString()+n.bF[1].getBits().toString();//非叶子节点
            }
            ...
            for(int m=0;m<nn.children.size();m++)
            {
                ...
                snn += nn.datas[m] +nn.children.get(m).nodeHash + nn.bF[0].getBits().toString()+nn.bF[1].getBits().toString();//非叶子节点
            }
